                ...
                if 'coords' in blade['flap_profiles'][afi]:
                    for j in range(n_Re):
                        for ind in range(n_ctrl):
                            ...
                            maxTS = self.wt_ref['assembly']['control']['maxTS'] #bem: this is the max tip speed for the BAR project, but I owuld like to figure out how to get this value directly from the yaml file
                            KinVisc = self.wt_ref['assembly']['global']['KinVisc'] # Kinematic viscosity (m^2/s)
                            SpdSound = self.wt_ref['assembly']['global']['SpdSound'] # speed of sound (m/s)
                            Re_loc = c*maxTS*rR/KinVisc
                            Ma_loc = maxTS*rR/SpdSound
                            #eta = blade['outer_shape_bem']['chord']['grid'][afi]
                            print('Run xfoil for span section at ' + str(eta*100.) + '%, flap deflection ' + str(fa) + ' deg, and Re equal to ' + str(Re_loc))
